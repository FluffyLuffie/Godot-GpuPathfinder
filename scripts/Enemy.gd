...
func _physics_process(delta: float) -> void:
	if cur_interval == 0:
		...
		for i in range(4):
			...
			# var neighbor_score: float = (int(c.r * 255.0 * c.a) << 16) + (int(c.g * 255.0 * c.a) << 8) + int(c.b * 255.0 * c.a)
		...
		if (best_step != -1):
			look_at(Globals.cave.map_to_world(cur_tile + offsets[best_step]) + Vector2(Globals.tile_size / 2.0, Globals.tile_size / 2.0))
